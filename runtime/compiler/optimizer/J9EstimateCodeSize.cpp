...
TR_J9EstimateCodeSize::realEstimateCodeSize(TR_CallTarget *calltarget, TR_CallStack *prevCallStack, bool recurseDown)
   ...
   //if (callExists)
   if (calltarget->_calleeSymbol) // Need to run the below code to inline partially anyway (even if there are no calls in this callee)
      {
...
*/
   ...
   if (true) // Need to run the below code to inline partially anyway (even if there are no calls in this callee)
      ...
      for (i = 0; i < maxIndex; ++i)
         ...
         if (flags[i].testAny(bbStart))
            ...
            if (addFallThruEdge)
               ...
               cfg->addEdge(currentBlock, newBlock, heapAlloc);
               addFallThruEdge = true;
               }
            else
               {
         ...
         if (flags[i].testAny(isBranch))
            ...
            switch (bc = bci.current())
               ...
               case J9BCathrow:
                  setupLastTreeTop(currentBlock, bc, i, cfg->getEnd()->asBlock(), calltarget->_calleeMethod, comp());
                  cfg->addEdge(currentBlock, cfg->getEnd(), heapAlloc);
                  addFallThruEdge = false;
                  break;
               case J9BCtableswitch:
                  {
      ...
      for (i = 0; i < (int32_t) tryCatchInfo.size(); ++i)
         ...
         for (int32_t j = handlerInfo->_startIndex; j <= handlerInfo->_endIndex; ++j)
            if (blocks[j])
               cfg->addExceptionEdge(blocks[j], blocks[handlerInfo->_handlerIndex], heapAlloc);
         }
...
/*
      ...
      int32_t maxBytecodeIndex = calltarget->_calleeMethod->maxBytecodeIndex();
      TR::Block *block = NULL;
      //TODO: sort the starts and then you will have the correct indeces...
      //TODO: Can't we do this without trMemory?
      TR_Array<TR::Block*> arrayOfBlocks(comp()->trMemory());
      int i = 0;
      int len = 0;
      for (TR::ReversePostorderSnapshotBlockIterator blockIt (cfg->getStartForReverseSnapshot()->asBlock(), comp()); blockIt.currentBlock(); ++blockIt)
         {
             block = blockIt.currentBlock();
             if (block->getBlockBCIndex() >= maxBytecodeIndex) continue;
             if (block->getBlockBCIndex() == -1) continue;
             if (arrayOfBlocks.size() == 0) {
                arrayOfBlocks.insert(block, 0);
                continue;
              }
             //TODO: no linear sort...
             int j = 0;
             for (j = 0; j < arrayOfBlocks.size(); j++) 
                {
                   TR::Block *currentBlock = arrayOfBlocks.element(j);
                   if (block->getBlockBCIndex() < currentBlock->getBlockBCIndex())
                      break;
                }
             arrayOfBlocks.insert(block, j);
             len++;
         }
      // Ok, now the blocks are in order in arrayOfBlocks.
      // We should make sure that the starts and ends are correctly set...
      for (int j = 0; j < arrayOfBlocks.size(); j++) 
         {
             TR::Block *currentBlock = arrayOfBlocks.element(j);
             auto num = currentBlock->getNumber();
             auto start = currentBlock->getBlockBCIndex();
             auto end = arrayOfBlocks.size() <= j + 1 ? maxBytecodeIndex : arrayOfBlocks.element(j+1)->getBlockBCIndex();
             currentBlock->setBlockSize(end - start);
         }
       // DONE!
...
*/
     ...
     if (calltarget->_calleeSymbol) calltarget->_calleeSymbol->setFlowGraph(cfg);
     return cfg;
   }
   return NULL;
   }
